...
HTMLComponents.prototype = {
    ...
    options: {
        ...
        /**
         * @member {String} default folder where the components are stored
         * @default 'components'
         */
        componentsFolder: 'components',

        /**
         * @member {String} source directory, srcDir is relative to the projectFolder
         * @exemple : 'app/'
         */
        srcDir: '',

        /**
         * @member {String} Destination directory, where the html files are generated, the destDir is relative to the projectFolder
         */
        destDir: '',

        /**
         * @member {String} Prefix of the attr when you need to use html content inside an attribute
         * @example
        ...
        /**
         * @mamber {Array} The match of the html files, base on the minimatch library
         * @default ['*.html']
         */
        files: ['*.html']
    },
    ...
    processHTML: function (html) {
        ...
        $('script').filter('[type="text/html"],[type="text/template"]').each(function () {
            var html = $(this).text();
            if (/^(?:[^#<]*(<[\w\W]+>)[^>]*$|#([\w\-]*)$)/.test(html)) {
                var re = (/<!\[CDATA\[([\s\S]+)\]\]>/);
